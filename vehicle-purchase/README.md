...
Assume only the kinds `'car'` and `'truck'` require a license, everything else
...
For that implement the function `chooseVehicle(option1, option2)` that takes
two vehicles as arguments and returns a decision that includes the option that
...
this logic using `if`, `else if` and `else` (there are other ways if you want
